...
function getEmbeddedInstallState() {
  ...
  if (isDev) {
    ...
    const venvFolder = fs.existsSync(path.join(rootDir, '.venv')) ? '.venv' : 'venv';
    const venvPython = process.platform === 'win32'
      ? path.join(rootDir, venvFolder, 'Scripts', 'python.exe')
      : path.join(rootDir, venvFolder, 'bin', 'python');
...
ipcMain.handle('embedded-server:install', async () => {
  if (process.platform === 'win32') {
    return { ok: false, error: 'Automatic setup is not yet supported on Windows. Please install manually by navigating to the embedded-server directory, creating a python venv, and running `pip install -r requirements.txt`.' };
  }
  const state = getEmbeddedInstallState();
  if (state.mode !== 'dev') {
