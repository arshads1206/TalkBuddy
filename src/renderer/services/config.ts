...
export const DEFAULTS = {
  stt: {
    ...
    groqUrl: 'https://api.groq.com/openai',
...
export interface GroqSTT { provider: 'groq'; url: string; model: string; apiKey: string; }
export type STTConfig = EmbeddedSTT | SpeachesSTT | GroqSTT;
...
export function resolveSTT(
  ...
  if (provider === 'groq') {
    return {
      provider: 'groq',
      url: p.groqSttUrl || DEFAULTS.stt.groqUrl,
      model: p.groqSttModel || DEFAULTS.stt.groqModel,
      apiKey: p.groqSttApiKey || p.groqApiKey || '',
    };
  }
