...
// The embedded server requires a per-session bearer token (generated by
// main, passed to the server via env) on everything except /health —
// otherwise any local process could use the microphone pipeline.
async function getEmbeddedAuthHeaders(): Promise<Record<string, string>> {
  const status = await getEmbeddedServerStatus();
  return status.token ? { Authorization: `Bearer ${status.token}` } : {};
}
...
export async function transcribeAudio(audioBlob: Blob): Promise<TranscriptionResult> {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/v1/audio/transcriptions`, {
      method: 'POST',
      headers: await getEmbeddedAuthHeaders(),
      body: formData,
    });
